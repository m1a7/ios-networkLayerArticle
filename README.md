...
![](Documentation/Carbon-Screens/scheme11.png)
...
В отличие от большинства работы посвященных темам архитектуры, которые являются по большей части теоретическими, данная статья изобилует примерами конкретных решений. 
...
Стандартный класс `NSURLSessionTask` (и его наследники) традиционно выполняющий сетевые запросы напротив же принципиально не используется в сетевом слое напрямую, по причине того, что поддержка различных протоколов для корректного функционирования объекта, является громоздкой и недостаточно гибкой.

К примеру если вы хотите во время получения данных из интернета отображать пользователю прогресс загрузки, при использовании `RXNetworkOperations` вам не потребуется реализовывать методы протокола, все необходимые данные вы можете получить в соответствующих блоках.  
...
![](Documentation/Carbon-Screens/scheme1.png)
...
В итоге при использовании стандартных подходов в построении сетевого слоя, а именно использование голых `NSURLSessionTask`, а также оберток по типу `AFNetworking` и прочих, у вас неизбежно будет образовываться `callback-hell `разной степени сложности.<img src="Documentation/Carbon-Screens/callbackhell.jpg" title="" alt="" data-align="center">                                                                           
...
<img src="Documentation/Carbon-Screens/scheme3.png" title="" alt="" data-align="center">`RXNO_GroupOperation` имеет некую схожесть с `NSBlockOperation`, и предназначен для того, чтобы в теле его `mainBlock` вы синхронно запускали выполнение обычных сетевых операций, для избежания возникновения `callback-hell`.
...
Фреймворк `RXNO` предоставляет выбор пользователю на основании какой сессии `NSURLSession` будет создана задача `NSURLSessionTask`.
По умолчанию для каждой операции фреймворк предоставляет свою внутреннею сессию, которая является проперти класса.
...
/*--------------------------------------------------------------------------------------------------------------
 ...
 Вы можете сделать это, вызывав данный метод внутри 'didFinishLaunchingWithOptions:..'.
 --------------------------------------------------------------------------------------------------------------*/
...
Ниже идет секция методов которые выполняют требования протокола `<Auth2_0_Delegate>`. 
...
Ниже приведены в пример несколько демонстрационных методов получающие данные пользователей.
...
В данном файле мы объявляем все `typedef` сокращения для классов и блоков, макрос `#define` и СИ-функции.
...
1. Вы можете получить нужный вам запрос используя общий метод `+buildRequestForMethod:properties:.`
...
2. Вы можете получить нужный вам запрос используя индивидуальную для каждого метода `API` функцию.
...
Весь код вы можете увидеть здесь: [NetworkRequestConstructor.h](CodeSnippets/NetworkRequestConstructor.h) / [NetworkRequestConstructor.m](CodeSnippets/NetworkRequestConstructor.m)

### 🖨🧾 Templater

| Главная задача          | Восстанавливать образцы ответов сервера в формате .json с диска устройства.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Список обязанностей** | - Взаимодействовать с классом `TemplaterFileManager` который осуществляет управление песочницей.<br/>- Записывать/Читать значения из `NSUserDefault`.<br/>- Инициализировать словари из `json` файлов находящихся в песочнице приложения.<br/>- Записывать шаблоны в песочницу по имени `API` метода, по которому был совершен запрос.<br/>- Удалять конкретный шаблон, по имени `API` метода, по которому был совершен запрос.<br/>- Удалять все шаблоны с диска.<br/>- Возможность безопасно перемещать папку с шаблонами в другие локации. |
...
Ниже мы можем увидеть как обрабатываются каждый ответ от сервера внутри `APIManager`.
После удачного выполнения сетевой операции мы вызываем метод валидации ответа, куда передаем сам `json` и значения енума, которое хранит название `API` метода. 
Далее `Validator` самостоятельно вызывает метод класса `Templater`, который будет пытаться восстановить образец корректного ответа сервера.
В случае если в хранилище устройства окажется нужный образец, произойдет сравнительная проверка.
Если она будет пройдена успешно, то метод валидации `validateResponse:fromAPIMethod:` вернет `nil`, в противном случае будет возвращена ошибка.

![](Documentation/Carbon-Screens/scheme5.png)
...
1. Если вы имеете намерения зашить правильные образцы ответов сервера в проект приложения, то самым удобным способом будет создания архива с именем `APIManagerResponseDefaultTemplates.zip`, который необходимо добавить в `Bundle` приложения. 
   А после вы должны вызывать безопасный метод распаковки `unarchiveFolderWithDefaultTemplates`, даже если вы вызовите его несколько раз то архив заново распакован не будет.
...
Весь код вы можете увидеть здесь: [Templater.h](CodeSnippets/Templater.h) / [Templater.m](CodeSnippets/Templater.m) .  [TemplaterFileManager.h](CodeSnippets/TemplaterFileManager.h) / [TemplaterFileManager.m](CodeSnippets/TemplaterFileManager.m)
<br>
...
2. **Ручной** - тогда сам разработчик решает как ему определять корректность или некорректность ответа.

Несомненно стоит отметить, что процесс валидации является очень специфическим. 
И не для всех `backend`'ов может подойти автоматический вариант.
...
Выше вы уже могли увидеть упоминания о методе выполняющим автоматическую проверку, теперь пришло время познакомиться с ним поближе.
Итак, один из главных параметров, которые принимает метод это битовая маска. 
...
Автоматические алгоритмы способны <u><mark>валидировать только словари</mark></u>. Сравнивать между собой два массива алгоритм не может, по причине невозможности установить какие объекты сравнивать друг с другом.
Поэтому если ваш сервер возвращает массив, а вы хотите использовать методы автоматической валидации, увы сделать этого не получиться. Только если создать полуавтоматический метод, который самостоятельно будет извлекать элементы из массива и передавать их в методы авт.валидации.

---

Если вы внимательно читали весь раздел выше, то могли заметить выражение "**словарь правил**".
Данный термин означает словарь в котором хранятся непосредственные параметры валидации <u>для конкретного проперти</u>.

**Приведем  пример**: 
Представим перед вами стоит задача валидировать ответ сервера на предмет возраста юзера. Чтобы ему было больше `18` но меньше `27`.
Тогда для решения этого вопроса создайте в шаблоне "`словарь с правилами`". 
...
Итак, изучив демонстрационный пример мы поняли что такое "**словарь правил**", и по какому принципу он работает. 
...
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 👉🏻`isOptional` | (По-умолчанию отсутствие ключа из шаблона в ответе сервера - воспринимается алгоритмом как ошибка).<br/>                     Значение `true` для этого ключа будет говорить алгоритму, что наличие пары (ключ-значение)<br/>                     для этого `json` объекта - является необязательным. |
| 👉🏻`mustMatch`  | Значение `true` требует соответствия значения переменной из шаблона, с тем что пришло с сервера.<br/>                    Данный ключ применим ко всем следующим типа данных (`Strings/Numbers/Dictionaries/Arrays`).                                                                                 |

**Пример**
Как мы можем видеть из шаблона ниже, то наличие значение по ключу `favouriteFilm` в пришедшем с сервера ответе, является необязательным.
...
Один из хрестоматийных примеров это валидация пришедшего кода подтверждения транзакции с сервера.
...
Как было показано во введении, числовые значения из `json` поддерживают только валидацию на минимальное и максимальное значение.
...
`Parser` по праву  можно назвать самым простым модулем в сетевом слое. 
Поскольку он применим исключительно в особых случаях, когда требуются достать глубоко расположенные данные.
...
Модуль `Mapper` можно называть вторым по легкости модулем сетевого слоя после `Parser`'a.
Поскольку маппинг данных является достаточно трудозатратным делом, поэтому внутри данного модуля имеет смысл подключить внешнею библиотеку.
Выбор библиотеки будет зависеть от языка вашего проекта, а также возможно от набора специфических требований.

Мы предпочитаем использовать библиотеку [FastEasyMapping](https://github.com/Yalantis/FastEasyMapping).

Одним из камней преткновения в реализации данного модуля является вопрос, где хранить методы описывающие механизм маппинга для конкретной модели.
Мы считаем, что подобный код должен размещаться в самой модели, чтобы модуль `Mapper` был максимально чистым и лаконичным.
...
Дорогие коллеги, прочитав все то, что написано выше вы ознакомились со всеми вспомогающими модулями, которые обеспечивают корректную работу `APIManager`'a.
Теперь вы готовы приступить к самой интересной части статьи, где будут непосредственно рассмотрены все аспекты управления сетевым слоем.
<br>
...
Для этого мы имеем определенный набор инструментов.
Одним из самых популярных и горячо любимых инструментов в нашем сообществе является `Reachability`, однако данная технология проверяет только наличие локальной сети. 
То есть если ваш wi-fi раздает сигнал, но не имеет непосредственного доступа к сети, то `Reachability` не вернет вам ни одной ошибки.
Поэтому я предлагаю вам взглянуть на эту таблицу, где аналогичные инструменты проверяющие доступность наличия сети, разбиты по определенным категориям.
...
Опять же как вы могли догадаться в конечном итоге выбор технологии зависит от языка вашего проекта.
В нашем сетевом слое мы используем [RealReachability](https://github.com/dustturtle/RealReachability), впрочем, если вы используете `swift`, то рекомендуем вам [Connectivity](https://github.com/rwbutler/Connectivity).
...
3. **Третим** вызывается метод класса `Validator`, который валидирует ответ сервера.
...
Традиционно для обработки ошибок возникших во время выполнения операции, в  классе `APIManager` используется метод `checkOnServerAndOtherError`.
Он обрабатывает широкий спектр ошибок. Если на ошибки по типу отмены операции или истекания времени выполнения реакция довольно-таки предсказуемая, просто вызывается `completion` блок и в него передаются соответствующие объекты.
То реакция на ошибку `401` значительно отличается своей сложностью решения.

**Краткое объяснение механизма действий при возникновении 401 ошибки**

Фреймворк **RXNO** в одном из своих классов имеет массив так называемых **"отложенных операций"**, если ваша операция была завершена с ошибкой аутентификации, то вы добавляете ее в данный массив.
После чего занимаетесь вопросами получения нового токена.
Как только получите новый токен, то сразу же вызывайте метод библиотеки который в своем блоке передаст в ваше распоряжение массив тех самый **"отложенных операций"**, а ваша задача вручную вставить в каждую из операций новый токен.
Далее вы передадите массив модифицированных операций в блок той функции, а она в свою очередь пошлет их на выполнение в очередь.

![](Documentation/Carbon-Screens/scheme7.png)
...
![](Documentation/Carbon-Screens/scheme8.png)

В коде метода `receiveTokenFromWebViewAuth` вы можете заметить на первый взгляд странные проперти по типу `isMayUnlockSemaphore` или методы `unlockSemaphoreForAllPostponned`. 
Дело заключается в том, что **RXNO** поддерживает синхронное выполнение операций.
Чтобы операция могла выполняться синхронно, для этого поток с которого она вызывает метод `syncStart` будет заблокирован технологией семафоров `GCD`, до тех пор, пока она не будет выполнена.
...
Также обратите внимание на следующий момент, если вы совершили запрос на сервер, он до него дошел, и вам был отправлен ответ с ошибкой (например отказ в доступе), тогда с точки зрения `NSURLSessionDataTask` операция завершилась успехом. Потому что запрос был отправлен, а ответ получен. А как раз таки в теле ответа и будет скрываться ошибка, которую мы должны обнаружить и отреагировать.
В коде расположенном внизу мы видим что производится проверка на наличие ключа `error` в джейсоне ответа.
Далее все происходит по известному нам сценарию.
...
Руководствуясь накопленным опытом разработки на свифт, можно сделать вывод, что практика создания отдельных категорий расширяющих класс - является достаточно эффективной.
...
Также подобные категории можно создавать например для поддержки шифрования и прочих функциональных обязанностей.
...
Конечно же очевидным является то, что практики взаимодействия с сетевым слоем могут существенно отличаться в разных архитектурных паттернах.
По моему мнению одним из самых сбалансированных, является паттерн **MVVM**, поскольку соотношение между количеством файлов и удобством использования у него является оптимальным.
Ниже будут продемонстрированы примеры работы сетевого слоя в архитектуре **MVVM**.

![](Documentation/Carbon-Screens/scheme9.png)

| Обязанности<br/>ViewController                              | Обязанности<br/>ViewModel                                                                                                                                                         | Обязанности<br/>APIManager                                                                      |
| ----------------------------------------------------------- |:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| - Хранить `viewModel`.<br/>- Обрабатывать `UI` события<br/> | - Хранить сетевые операции.<br/>- Иметь методы обертки (которые обращаются к `APIManager` за созданием операций).<br/>- Иметь метод отменяющий работу всех сетевых операций.<br/> | - Создавать и конфигурировать сетевые операции.<br/>- Отвечать за обновление и хранение токена. |
...
1. Проперти на сетевые операции **ВСЕГДА** должны иметь аттрибут `__block`.
   Это нужно потому, что в первый раз операция может завершиться неудачно (с ошибкой `401`) и с помощью внутреннего метода-помощника `checkOnServerAndOtherError` в классе `APIManager` она автоматически будет отложенна и выполненна еще раз, после того как `APIManager` самостоятельно запросит и получит свежый токен. 
   Поскольку каждая сетевая операция является наследником класса `NSOperation`, а одну и ту же операцию невозможно вставить в очередь более одного раза.
   Поэтому внутри метода `performPostponedOperationsOnQueue` операции пересоздаются, импортируя все параметры их предшествеников. 
   Следовательно, если ваше проперти не будет иметь аттрибута `__block`, то в случае неудачного завершения `401` и повторного успешного выполнения, ваша `viewModel`  будет удерживать ссылку именно на первую реинкарнацию операции.
...
2. Для каждой сетевой операции внутри `viewModel` помимо проперти также должен иметься индивидуальный метод-обертка, который обращается к `APIManager` для инициализирования данной операции и обрабатывает `completion` блок того метода конструктора.
   Это нужно для того, чтобы получив готовые модели данных в `completion` блоке, вы могли преобразовать их во внутренние `viewModel` (например для ячеек таблицы) и добавить их к примеру во внутренние массив данных.
...
3. Вне зависимости от того насколько большую функциональную нагрузку имеет абстрактный контроллер, содержит ли он проперти всего на одну сетевую операцию или на десяток.
   Мы должны обязательно иметь проперти на групповую операцию, которую необходимо выполнять каждый раз, когда данный контроллер представляется на экране устройства.
...
4. По умолчанию метод в котором инициализируется групповая операция, называется `performNeededOperations`. 
   Именно этот метод надо вызывать внутри `viewDidLoad` вашего контроллера.
...
2. Как вы могли видеть ранее, каждый метод-обертка имеет два одинаковых параметра `runOpItself` и `queue`. Они нужны для того, чтобы метод обертка самостоятельно решил надо ли запускать выполнения операции или нет, ставить в очередь или нет.
...
3. Установите идентификатор для вашей сетевой операции, чтобы ее потом можно было удалить из очереди (`NSOperationQueue`) до начала ее фактического исполнения.
   Такая функциональная возможность может пригодиться когда вы выходите с экрана, от имени которого в очередь были добавлены операции.
...
4. Вызовите все необходимые методы-обертки внутри `performNeededOperations`.
...
[UserProfileTVC.h](CodeSnippets/UserProfileTVC.h) / [APIManager+Utilites.m](CodeSnippets/UserProfileTVC.m).
...
Ниже будет расписана инструкция по добавлению поддержки методов вашего `API` в `APIManager`.
...
6. Создайте соответствующий метод, который будет валидировать ответ сервера.
...
9. Если сервер конкретно по запросу на метод, поддержку которого вы собираетесь добавить, возвращает крайне сложную структуру данных. 
   И вы испытываете сложности в извлечении нужной структуры, чтобы потом ее передать в `Mapper`.
   То тогда создайте и вынесите этот код в метод, который будет находиться в модуле `Parser`.
...
11. Создайте соответсвующий метод.
...
Дорогие коллеги, абсолютно ясным является тот факт, что каждое приложение имеет свою направленность, относительно которой и развивается ее сетевой слой. Который обязательно имеет некую определенную специфику.

Данная реализация сетевого слоя показывает лишь общие моменты для большинства приложений. 
