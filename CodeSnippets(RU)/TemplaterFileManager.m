...
@implementation TemplaterFileManager
...
{
    return [[TemplaterFileManager pathForApplicationSupportDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[TemplaterFileManager pathForCachesDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[TemplaterFileManager pathForDocumentsDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[TemplaterFileManager pathForLibraryDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[TemplaterFileManager pathForMainBundleDirectory] stringByAppendingPathComponent:path];
}
...
{
    return [[TemplaterFileManager pathForTemporaryDirectory] stringByAppendingPathComponent:path];
}
...
+(NSObject *)readFileAtPathAsCustomModel:(NSString *)path
{
    ...
    NSError* error = nil;
    NSData*   data = [NSData dataWithContentsOfFile:[self absolutePath:path]];
    NSObject* file = nil;
    
    if (@available(iOS 11, *)) {
        file = [NSKeyedUnarchiver unarchivedObjectOfClass:[NSObject class] fromData:data error:&error];
    }
    return file;
}
...
+(BOOL)writeFileAtPath:(NSString *)path content:(NSObject *)content error:(NSError **)error
{
    ...
    else if([content conformsToProtocol:@protocol(NSCoding)])
    {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] <= 12.0) {
            ...
            NSError *error = nil;
            NSData *data = [NSKeyedArchiver archivedDataWithRootObject:content requiringSecureCoding:NO error:&error];
            [data writeToFile:absolutePath options:NSDataWritingAtomic error:&error];
        }
    }
    else {
