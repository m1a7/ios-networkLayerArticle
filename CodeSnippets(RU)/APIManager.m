            ...
            for (NSData* imageData in attachments) {
                [[APIManager uploadImages:@[imageData]
                               ...
                               completion:^(NSArray<NSDictionary *> * _Nullable savedImages, GO *uploadGroupOp) {
                                   ...
                                   // Впоследствии строку 'stringAttachments' мы передаедим в метод 'wallPost',
                                   // Чтобы эти адреса этих фотографий были прикреплены к посту
...
/*--------------------------------------------------------------------------------------------------------------
 ...
 адреса УЖЕ РАНЕЕ загруженных фотографий.
 --------------------------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------------------------
 ...
 Из принятого аргумента 'op' - алгоритм выявляет ошибки возникшие во время выполнения операции.
    ...
    @synchronized ([NSNotificationCenter defaultCenter])
    {
        if (!_defaultSession){
            ...
            if (@available(macOS 10.13.1, iOS 11, *)) {
                ...
                // В противном случае ожидание будет осуществляться до превышения значения из 'timeoutIntervalForResource'.
